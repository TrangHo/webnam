  ...
  class PasswordsController < Devise::PasswordsController
    ...
    def give_notice
      if %w(notice error alert).exclude?(flash.keys.map(&:to_s)) or @refinery_user.errors.any?
        flash[:notice] = ::I18n.t('successful', :scope => 'refinery.users.reset', :email => @refinery_user.email)
      end
    end
    ...
    def edit
      if params[:reset_password_token] and (@refinery_user = User.where(:reset_password_token => params[:reset_password_token]).first).present?
        ...
        redirect_to refinery.new_refinery_user_password_path,
                    :flash => ({ :error => ::I18n.t('code_invalid', :scope => 'refinery.users.reset') })
      end
    end
    ...
    def create
      if params[:refinery_user].present? and (email = params[:refinery_user][:email]).present? and
        ...
        redirect_to refinery.new_refinery_user_session_path,
                    :notice => ::I18n.t('email_reset_sent', :scope => 'refinery.users.forgot')
      else
        flash.now[:error] = if (email = params[:refinery_user][:email]).blank?
          ::I18n.t('blank_email', :scope => 'refinery.users.forgot')
        else
          ::I18n.t('email_not_associated_with_account_html', :email => ERB::Util.html_escape(email), :scope => 'refinery.users.forgot').html_safe
        end
        render :new
      end
    end
